    ...
    <div id='wrapper'>
      <div className='prodSideBar flex-col mt-6 ml-4 max-w-2/6 max-h-full'>
        ...
        <div className='prodFeat flex p-4 max-w-2/6 max-h-2/5'>
          <div className='prodIconHead flex flex-col'>
            {product?.productFeatures?.map((feature) => (
              <img key={feature?.name} className='prodIcon p-0.5 mb-0.5 w-9 h-6 max-w-9 max-h-6 bg-iconBg' src={feature?.icon} />
            ))}
          </div>
          <div className='prodFeatHead w-full flex flex-col'>
            {product?.productFeatures?.map((feature) => (
              <div key={feature?.name} className='prodFeatContent p-0.5 ml-0.5 mb-0.5 text-xs w-full text-buttonColor bg-blueItemBg'>
                <span>
        ...
        <div className='gameAchievement p-4 text-greyFontColor text-xs'>
          ...
          {gameData ? gameData.achievement_images.map((_, index) => <img key={index} src={gameData.achievement_images[index]} className='mt-4 w-16 h-16' />) : null}
        </div>
