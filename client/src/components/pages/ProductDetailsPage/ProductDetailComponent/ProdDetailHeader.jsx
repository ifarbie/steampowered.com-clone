      ...
      <div id='headerBg' className='flex flex-row-reverse pb-4 max-w-full max-h-full'>
        <div id='sideDesc' className='ml-4 min-w-60 w-screen max-w-full max-h-full'>
          ...
          <div className='genreTag mt-2 mb-2 text-xs w-full'>
            ...
            {product?.Categories?.map((category, index) => {
              return (
                <span key={index} className='text-buttonColor bg-greyBg rounded-sm py-1 px-1.5 mr-0.5'>
                  {category.name}
                </span>
              );
            })}
        ...
        <div id='mainVideo' className='w-full h-full'>
          {mainContent.type === 'video' ? <video controls src={mainContent.src} type='video/mp4'></video> : <img src={mainContent.src} alt='Main Preview' className='w-full' />}
              ...
              {product?.ScrollThumbnails?.map((thumbnail, index) => (
                <SwiperSlide key={index}>
                  {thumbnail.type === 'video' ? (
                    <video src={thumbnail.img} type={thumbnail.type} className={`thumbnail ${activeSlide === index ? 'border-white border-2' : ''}`} onClick={() => handleThumbnailClick(thumbnail.img, 'video', index)}></video>
                  ) : (
                    <img
                      src={thumbnail.img}
                      type={thumbnail.type}
                      alt={`Screenshot ${index + 1}`}
                      className={`thumbnail ${activeSlide === index ? 'border-white border-2' : ''}`}
